...
exports.autoGenerate = (function (dir) {
    ...
    function getDirDependence(dir) {
        ...
        for (var i = 0, len = files.length; i < len; i++) {
            ...
            if (-1 === excludes.indexOf(file) && /\.js/.test(file)) {
                ...
                position = file.lastIndexOf('/',file) + 1
                name = file.substring(position);
                dependencies.push(_jsonTemplate(filename, getDependence(dir + '/' + file),name));
            }
        }
        return dependencies;
    }
     ...
     * @private
     */
    function _jsonTemplate(file, dependencies,name) {
        var temp = {
            src: file,
            dest: getDest(file),
            name: name
